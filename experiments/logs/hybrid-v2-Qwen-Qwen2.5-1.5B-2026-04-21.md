...
- Router sensitivity note: the newly added keywords (`from_numpy`, `numpy array`, `share memory`) are specific enough that they should not accidentally re-route other items. This was verified on both benchmarks:
  - core: Session B confirmed only `tensor_creation_002` flipped among the 36 items.
  - smoke: `smoke-2026-04-21-102219.json` is byte-identical to the 2026-04-20 smoke baseline (`smoke-2026-04-20-212010.json`) on every dimension checked — summary numbers, per-category accuracy, all 12 labels, all 12 answer strings, all 12 `retrieval_debug` blocks, and the full results-block fingerprint (`2aa2e0c3b30214c2` vs `2aa2e0c3b30214c2`, ignoring timestamp). No smoke query contains `numpy`, `from_numpy`, or `share memory`, so the new keywords never triggered on the smoke set.
  - Any future keyword addition to `EXACT_PROMPT_QUERY_KEYWORDS` should still be done one keyword at a time with a full core + smoke rerun in between.
