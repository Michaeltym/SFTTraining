...
After changing any `.jsonl` file, always verify the file contents before finishing the task.

At minimum, check:

- the file is valid JSONL
- every row has the expected fields
- there are no accidental duplicates unless duplicates are intentional
- the facts, symbols, shapes, and API names are correct
- the changed rows still match the intended split purpose such as training, validation, or benchmark
